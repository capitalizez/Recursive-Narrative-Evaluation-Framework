...
# CSV prompt loader
def load_prompts(path='tests/recursive_prompts.csv'):
    try:
...
def random_prompt():
    return random.choice(PROMPT_POOL) if PROMPT_POOL else "Hello?"
...
def chat(user_message, history, temperature, max_tokens, model_name, context_file):
    ...
    next_prompt = random_prompt()
    return history, snippet, next_prompt
...
with gr.Blocks(css=dark_css) as demo:
    ...
    with gr.Row():
        with gr.Column(scale=3):
            ...
            user_input = gr.Textbox(value=random_prompt(), placeholder="Type your message here...", label="Your message")
